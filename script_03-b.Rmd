...
increase <- inner_join(sep_old, sep_new, by = "id", suffix = c("_old", "_new")) %>%
  mutate(delta = u_grad_new - u_grad_old) %>%
  arrange(desc(delta)) %>%
  select("name_old", "department_old","delta") %>%
head(5) %>%
  gt() %>%
  tab_header(title = "Top 5 Classes with Largest Enrollment Increases") %>%
  cols_label(name_old = "Name",
...
decrease <- inner_join(sep_old, sep_new, by = "id", suffix = c("_old", "_new")) %>%
  mutate(delta = u_grad_new - u_grad_old) %>%
  arrange((delta)) %>%
  select("name_old", "department_old","delta") %>%
head(5) %>%
  gt() %>%
  tab_header(title = "Top 5 Classes with Largest Enrollment Decreases") %>%
  cols_label(name_old = "Name",
             department_old = "Department",
             delta = "Change")
decrease
...
full_join(increase, decrease, by = c("delta", "name_old", "department_old")) %>%
arrange(delta) %>%
  ggplot(aes(x = name_old, y = delta, fill = department_old)) +
  geom_col() +
  labs(title = "Courses By Largest Enrollment Changes",
       subtitle = "Gen Eds are unpopular",
       x = "Name", y = "Change") +
  coord_flip()
